...
pub struct Node<K, V> {
    ...
    pub children: Vec<i64>,     // Offsets into the memmap file
    pub max_keys: usize,        // Maximum number of keys a node can hold
    pub node_type: NodeType,
    pub offset: i64,
    pub is_root: bool,
    pub parent_offset: Option<i64>, // Offset of the parent node
}
    ...
    V: Clone + TreeSerialization + TreeDeserialization,
{
    ...
    pub fn split(&mut self, b: usize) -> Result<(K, Node<K, V>), HaystackError> {
        // println!("Splitting node: {:?}", self.keys);

        match self.node_type {
            NodeType::Internal => {
                if b <= 1 || b > self.keys.len() {
                    return Err(HaystackError::new("Invalid split point for internal node"));
                }
            ...
            NodeType::Leaf => {
                if b < 1 || b >= self.keys.len() {
                    return Err(HaystackError::new("Invalid split point for leaf node"));
                }
    ...
    pub fn deserialize(data: &[u8]) -> Self {
        ...
        offset += 1;
        let parent_offset = i64::from_le_bytes(data[offset..offset + 8].try_into().unwrap()) as i64;
        offset += 8;
        ...
        for _ in 0..num_children {
            let child_offset = i64::from_le_bytes(data[offset..offset + 8].try_into().unwrap());
            offset += 8;
